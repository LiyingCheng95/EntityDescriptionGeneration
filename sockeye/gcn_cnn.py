...
class GraphConvolution:
    ...
    def __init__(self,
        ...
        self._output_dim = output_dim
        self._direction_num = direction_num
        utils.check_condition(output_dim % self._heads == 0,
                              "Number of sublayers (%d) must divide attention depth (%d)" % (self._heads, output_dim))
        ...
        # Each layer in the sub-block takes the input from all previous layers, so the input dimension is increased by hidden_dim * i for the ith sublayer
        # Each direction has a separate DCGCN to encode (e.g. for AMR graph we have 3 DCGCNs, whose parameters are not shared)
        ...
        self._direct_W = [mx.symbol.Variable(self._prefix + str(i) + '_direct_weight',
                                             shape=(self._direction_num * self._hidden_dim, self._direction_num * self._hidden_dim))
                          for i in range(self._heads)]
        self._direct_b = [mx.symbol.Variable(self._prefix + str(i) + '_direct_bias',
                                             shape=(self._direction_num * self._hidden_dim,))
                          for i in range(self._heads)]


        # MLP Params for aggregating the output of each direction
        self._MLP_W = [mx.symbol.Variable(self._prefix + str(i) + '_mlp_weight',
                                             shape=(self._direction_num * self._hidden_dim, self._hidden_dim))
                          for i in range(self._heads)]
        self._MLP_b = [mx.symbol.Variable(self._prefix + str(i) + '_mlp_bias',
                                             shape=(self._hidden_dim,))
                          for i in range(self._heads)]
    ...
    def _convolve(self, adj, inputs, i):
        ...
        outputs = mx.sym.Activation(outputs, act_type=self._activation)
        outputs = mx.sym.dot(outputs, self._MLP_W[i])
        outputs = mx.sym.broadcast_add(outputs, self._MLP_b[i])
        ...
        if self._norm:
            ...
            final_output = mx.sym.broadcast_div(outputs, norm_factor)
