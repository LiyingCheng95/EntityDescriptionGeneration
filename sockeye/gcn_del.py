...
class GCNCell(object):
    def __init__(self,
        ...
        #                                        shape=(self._num_layers * self._output_dim, self._output_dim))
...
class GraphConvolution:
    ...
    def _convolve(self, adj, inputs, i):
        ...
        for j in range(self._directions):
            ...
            dir_output = mx.sym.expand_dims(output, axis=1)
            direct_list.append(dir_output)

        outputs = mx.sym.concat(*direct_list, dim=1)
        outputs = mx.sym.mean(outputs, axis=1)
