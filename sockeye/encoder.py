...
class GraphConvolutionEncoderConfig(config.Config):
	def __init__(self,
...
def get_gcn_encoder(config: GraphConvolutionEncoderConfig, prefix: str) -> 'Encoder':
	encoder_seq = EncoderSequence([], dtype=config.dtype)
	cls, encoder_params = _get_positional_embedding_params(config.positional_embedding_type,
	...
	new_gcn_config = gcn.GCNConfig(input_dim=config.num_embed + config.pos_num_embed,
								   output_dim=config.gcn_config.output_dim,
								   direction_num=config.gcn_config.direction_num,
								   num_blocks=config.gcn_config.num_blocks,
								   adj_norm=config.gcn_config.adj_norm,
								   activation=config.gcn_config.activation,
								   dropout=config.gcn_config.dropout)

	encoder_seq.append(GraphConvolutionEncoder, config=new_gcn_config, prefix=C.GCN_PREFIX)
...
class ConcatLearnedPositionalEmbeddings(PositionalEncoder):
...
def _get_positional_embedding_params(positional_embedding_type: str,
	...
	elif positional_embedding_type == C.GRAPH_POSITIONAL_EMBEDDING:
		return ConcatLearnedPositionalEmbeddings, dict(num_embed=num_embed,
													   max_seq_len=max_seq_len,
...
class EncoderSequence(Encoder):
	...
	def encode(self,
		...
		for encoder in self.encoders:
			data, data_length, seq_len = encoder.encode(data, data_length, seq_len, metadata=metadata)
		return data, data_length, seq_len
