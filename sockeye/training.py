...
from . import utils
from . import vocab
from .encoder import EmptyEncoderConfig, RecurrentEncoderConfig, GraphConvolutionEncoderConfig
...
class TrainingModel(model.SockeyeModel):
    ...
    def _initialize(self,
        ...
        data_names = [C.SOURCE_NAME, C.TARGET_NAME, C.SOURCE_GRAPHS_NAME, C.SOURCE_POSITIONS_NAME]
        ...
        def sym_gen(seq_lens):
            ...
            # print(self.encoder.gcn_config)
            (source_encoded,
             source_encoded_length,
             source_encoded_seq_len) = self.encoder.encode(source_embed, source_embed_length,
                                                           seq_len=source_embed_seq_len,
    ...
    def _generate_fixed_param_names(self, param_names: List[str], strategy: str) -> List[str]:
        ...
        elif isinstance(self.config.config_encoder, GraphConvolutionEncoderConfig):
            num_encoder_layers = self.config.config_encoder.new_gcn_config.num_layers         
        else:
    ...
    def load(fname: str) -> 'TrainState':
        ...
        with open(fname, "rb") as fp:
            ...
            if not hasattr(state, '_time_elapsed'):
                ...
                logger.warning("Training state did not pr the elapsed time. Will assume take the absolute time "
