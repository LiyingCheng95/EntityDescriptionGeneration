...
class RawParallelDatasetLoader:
	...
	def load(self,
		...
		data_src_graphs = [[] for _ in num_samples_per_bucket]
		data_src_positions = [[] for _ in num_samples_per_bucket]
		...
		for sentno, (sources, target, src_graph) in enumerate(parallel_graph_iter(source_iterables, target_iterable, graph_iterable), 1):
			...
			data_src_graphs[buck_index].append(src_graph)
			data_src_positions[buck_index].append([])
	...
	def _get_graph_positions(self, bucket_size, data_src_graphs, global_index_list):
		...
		for i, adj in enumerate(data_src_graphs):
			...
			positions[i][global_index_list[i]] = -1000
		return positions
...
def get_validation_data_iter(data_loader: RawParallelDatasetLoader,
	...
	validation_sources_sentences, validation_target_sentences, validation_graphs = create_graph_readers(validation_sources,
																										validation_target,
																										val_src_graphs,
																										source_vocabs,
																										target_vocab,
...
def get_training_data_iters(sources: List[str],
	...
	sources_sentences, target_sentences, graphs = create_graph_readers(sources, target, source_graphs, source_vocabs, target_vocab, edge_vocab)
	...
	data_loader = RawParallelDatasetLoader(buckets=buckets,
										   eos_id=target_vocab[C.EOS_SYMBOL],
										   pad_id=C.PAD_ID,
										   forward_id=edge_vocab['d'])

	training_data = data_loader.load(sources_sentences, target_sentences, graphs,
...
class GraphReader(Iterable):
	...
	def __iter__(self):
		for graph_tokens in read_content(self.path, self.limit):
			graph = process_edges(graph_tokens, self.vocab)
			yield graph
...
def parallel_graph_iter(source_iters: Sequence[Iterable[Optional[Any]]], target_iterable: Iterable[Optional[Any]],
				  graph_iterable: Iterable[Optional[Any]]):
	"""
	...
	graph_iter = iter(graph_iterable)
	for sources, target, graph in zip(zip(*source_iters), target_iter, graph_iter):
...
class ParallelDataSet(Sized):
	...
	def __init__(self,
				 ...
				 src_graphs: List[mx.nd.array],
				 src_positions: List[mx.nd.array]) -> None:
		check_condition(len(source) == len(target) == len(label) == len(src_graphs) == len(src_positions),
	...
	def load(fname: str) -> 'ParallelDataSet':
		...
		target = data[n:2 * n]
		label = data[2 * n:]
		src_graph = data[3 * n:4 * n]
		src_position = data[4 * n:]
		assert len(source) == len(target) == len(label) == len(src_graph) == len(src_position)
		return ParallelDataSet(source, target, label, src_graph, src_position)
	...
	def fill_up(self,
		...
		graph = list(self.src_graphs)
		position = list(self.src_positions)
		...
		for bucket_idx in range(len(self)):
			...
			bucket_graph = self.src_graphs[bucket_idx]
			bucket_position = self.src_positions[bucket_idx]
			...
			if num_samples % bucket_batch_size != 0:
				...
				graph[bucket_idx] = mx.nd.concat(bucket_graph, bucket_graph.take(desired_indices), dim=0)
				position[bucket_idx] = mx.nd.concat(bucket_position, bucket_position.take(desired_indices), dim=0)
...
class ParallelSampleIter(BaseParallelSampleIter):
	...
	def __init__(self,
		...
		super().__init__(buckets=buckets, batch_size=batch_size, bucket_batch_sizes=bucket_batch_sizes,
						 source_data_name=source_data_name, target_data_name=target_data_name,
						 src_graphs_name=src_graphs_name, src_positions_name=src_positions_name,
						 label_name=label_name, num_factors=num_factors, permute=permute, dtype=dtype)

		# create independent lists to be shuffled
		self.data = ParallelDataSet(list(data.source), list(data.target), list(data.label),  list(data.src_graphs), list(data.src_positions))
	...
	def next(self) -> mx.io.DataBatch:
		...
		src_graphs = self.data.src_graphs[i][j:j + batch_size]
		src_positions = self.data.src_positions[i][j:j + batch_size]
