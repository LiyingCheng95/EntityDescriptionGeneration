...
class InferenceModel(model.SockeyeModel):
        ...
        def sym_gen(source_seq_len: int):
            ...
            (source_encoded,
             ...
             source_encoded_seq_len) = self.encoder.encode(source_embed,
                                                           source_embed_length,
                                                           source_embed_seq_len,metadata=(source_graphs, source_positions))
    ...
    def _get_encoder_data_shapes(self, bucket_key: int, batch_size: int) -> List[mx.io.DataDesc]:
        ...
        return [mx.io.DataDesc(name=C.SOURCE_NAME,
                ...
                mx.io.DataDesc(name=C.SOURCE_GRAPHS_NAME,
                               shape=(self.batch_size, bucket_key, bucket_key),
                               layout=C.BATCH_MAJOR),
    ...
    def run_encoder(self,
                    ...
                    source_positions: mx.nd.NDArray) -> Tuple['ModelState', mx.nd.NDArray]:
        """
    ...
    def chunks(self, chunk_size: int) -> Generator['TranslatorInput', None, None]:
        ...
        for chunk_id, i in enumerate(range(0, len(self), chunk_size)):
            ...
            yield TranslatorInput(sentence_id=self.sentence_id,
                                  ...
                                  graph=self.graph[i:i + chunk_size],
                                  factors=factors,
...
def make_input_from_plain_string(sentence_id: SentenceId, string: str, graph, edge_vocab) -> TranslatorInput:
    """
    ...
    """
    tokens = list(data_io.get_tokens(string))
    edge_list = list(data_io.get_tokens(graph))
    edges = data_io.process_edges(edge_list, edge_vocab)
    return TranslatorInput(sentence_id, tokens=tokens, graph=edges, factors=None)
...
def make_input_from_factored_string(sentence_id: SentenceId,
    ...
    if model_num_source_factors == 1:
        return make_input_from_plain_string(sentence_id=sentence_id, string=factored_string, graph=graph, edge_vocab=translator.vocab_edge)
...
def make_input_from_multiple_strings(sentence_id: SentenceId, strings: List[str], graph: str, edge_vocab: vocab.Vocab) -> TranslatorInput:
    """
    ...
    edge_list = list(data_io.get_tokens(graph))
    edges = data_io.process_edges(edge_list, edge_vocab)
    return TranslatorInput(sentence_id=sentence_id, tokens=tokens, graph=edges, factors=factors)
...
class Translator:
    ...
    def _get_inference_input(self,
        ...
        new_graph = mx.nd.zeros((len(trans_inputs), bucket_key, bucket_key), ctx=self.context)
        positions = np.ones((len(trans_inputs), bucket_key)) * 1000
        ...
        for j, trans_input in enumerate(trans_inputs):
            ...
            for tup in trans_input.graph:
                if (tup[0] < bucket_key) and (tup[1] < bucket_key):
                    new_graph[j][tup[0]][tup[1]] = tup[2] + 1
                    if tup[0] == tup[1]:
                        self_id = tup[2] + 1

            for k in range(bucket_key):
                new_graph[j][k][k] = self_id

            adj = new_graph.asnumpy()[j]
            dist = 0
            curr_node = self._find_root(adj)
            positions[j][curr_node] = dist
            forward_mask = np.ones_like(adj) * (self.forward_id + 1)
            forward_adj = (forward_mask == adj)
            self._fill_pos(dist+1, curr_node, positions[j], forward_adj)
        ...
        return source, bucket_key, new_graph, positions, restrict_lexicon, raw_constraints, raw_avoid_list, \
    ...
    def _encode(self, sources: mx.nd.NDArray, source_length: int, source_graph: mx.nd.NDArray, source_positions: mx.nd.NDArray) -> Tuple[List[ModelState], mx.nd.NDArray]:
        """
        ...
        for model in self.models:
            state, ratio = model.run_encoder(sources, source_length, source_graph, source_positions)
    ...
    def _beam_search(self,
        ...
        model_states, estimated_reference_lengths = self._encode(source, source_length, source_graph, source_positions)
