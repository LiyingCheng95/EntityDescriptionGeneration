...
def create_data_iters_and_vocabs(args: argparse.Namespace,
    ...
    if args.prepared_data is not None:
        ...
        train_iter, validation_iter, config_data, data_info = data_io.get_training_data_iters(
            ...
            validation_sources=validation_sources,
            validation_target=validation_target,
            val_source_graphs=os.path.abspath(args.val_source_graphs),
        ...
        return train_iter, validation_iter, config_data, source_vocabs, target_vocab, edge_vocab
...
def create_graph_encoder_config(args: argparse.Namespace,
                                vocab_edge_size: int,
                                max_seq_len_source: int) -> Tuple[encoder.EncoderConfig, int]:
    ...
    if args.encoder == C.GCN_TYPE:
        config_encoder = encoder.GraphConvolutionEncoderConfig(
            gcn_config=gcn.GCNConfig(input_dim=num_embed_source,
                                     ...
                                     direction_num=vocab_edge_size,
                                     num_blocks=args.gcn_num_layers,
                                     adj_norm=args.gcn_adj_norm,
                                     activation=args.gcn_activation,
                                     dropout=args.gcn_dropout),
            positional_embedding_type=args.gcn_positional_embedding_type,
            num_embed=num_embed_source,
            embed_dropout=encoder_embed_dropout,
            pos_num_embed=args.gcn_pos_embed,
...
def train(args: argparse.Namespace, custom_metrics_logger: Optional[Callable] = None) -> training.TrainState:
    ...
    with ExitStack() as exit_stack:
        ...
        train_iter, eval_iter, config_data, source_vocabs, target_vocab, edge_vocab = create_data_iters_and_vocabs(
            args=args,
            max_seq_len_source=max_seq_len_source,
            max_seq_len_target=max_seq_len_target,
        ...
        training_state = trainer.fit(train_iter=train_iter,
                                     ...
                                     decoder=create_checkpoint_decoder(args, exit_stack, edge_vocab, context),
